#!/usr/bin/env python3
"""Watchdog: wait until Qwen3-32B API is callable, then launch OLD-6 two-group sweeps.

Probe target (must match planned deploy on 172.18.30.162:8000):
  config key Qwen3-32b_think -> model_name == qwen3-32b
  GET /v1/models contains qwen3-32b
  POST /v1/chat/completions succeeds

Also probes Kimi-k2.5_think (needed by group B) before launch.

When ready (SUCCESS_STREAK consecutive healthy polls):
  bash tools/start_old6_qwen32b_two_group_macro.sh
    - all_think:   naming=ordering=executor=Qwen3-32b_think, conc=5
    - kimi_order:  naming=ordering=Kimi-k2.5_think, executor=Qwen3-32b_think, conc=3

Usage:
  python tools/watchdog_qwen32b_old6_macro.py
  python tools/watchdog_qwen32b_old6_macro.py --once --dry-run
  python tools/watchdog_qwen32b_old6_macro.py --self-test
"""
...
LAUNCH_SCRIPT = ROOT / "tools" / "start_old6_qwen32b_two_group_macro.sh"
STATE_DIR = ROOT / "game_records" / "_watchdog"
STATE_PATH = STATE_DIR / "qwen32b_old6.state.json"
LOG_PATH = STATE_DIR / "qwen32b_old6.log"

# tag -> config_key -> expected served model id
TARGETS: List[Tuple[str, str, str]] = [
    ("qwen32b", "Qwen3-32b_think", "qwen3-32b"),
    ("kimi_think", "Kimi-k2.5_think", "kimi-k2.5"),
]
...
def probe_endpoint(
    ...
    *,
    ...
    if not models_ok:
        ...
        if code_m == 200:
            # surface what is actually served (e.g. still mix-grpo)
            served = []
    ...
    # Qwen3 thinking off for health; Kimi disable thinking for a cheap probe.
    if "qwen" in ep.model_name.lower():
...
def launch_experiments(*, dry_run: bool = False, skip_completed: int = 0) -> int:
    ...
    env["AUTO_EXIT"] = "1"
    # per-group concurrency is baked into the launch script defaults
    args = ["bash", str(LAUNCH_SCRIPT), "all"]
    cmd_s = " ".join(args)
    if dry_run:
        log(f"[dry-run] would launch: {cmd_s}")
        return 0
    log(f"Launching: {cmd_s}")
...
def run_watch(
    *,
    ...
    watch_eps = endpoints
    if qwen_only:
        watch_eps = [e for e in endpoints if e.tag == "qwen32b"]

    log(
        f"Watch start: {[e.tag for e in watch_eps]} | interval={interval}s | "
    ...
    while True:
        ...
        if all_ready and all(
            state.streaks.get(ep.tag, 0) >= streak_need for ep in watch_eps
        ):
...
def self_test() -> int:
    ...
    try:
        ...
        qwen = next(e for e in eps if e.tag == "qwen32b")
        check(
            qwen.api_url == "http://172.18.30.162:8000/v1",
            f"qwen url={qwen.api_url}",
        )
        check(qwen.model_name == "qwen3-32b", f"qwen model={qwen.model_name}")
        kimi = next(e for e in eps if e.tag == "kimi_think")
        check(kimi.model_name == "kimi-k2.5", f"kimi model={kimi.model_name}")
    except Exception as e:
    ...
    check(
        _models_contain({"data": [{"id": "qwen3-32b"}]}, "qwen3-32b"),
        "models match qwen3-32b",
    )
    check(
        not _models_contain(
            {"data": [{"id": "sc2-mix-grpo-cot-only"}]}, "qwen3-32b"
        ),
        "reject mix-grpo as qwen3-32b",
    )
    ...
    if eps:
        ...
        good_m = (200, {"data": [{"id": "qwen3-32b"}]}, "ok")
        good_c = (
            200,
            {"choices": [{"message": {"content": "OK"}, "finish_reason": "stop"}]},
            "ok",
        )
        wrong_m = (200, {"data": [{"id": "sc2-mix-grpo-cot-only"}]}, "ok")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Watchdog for Qwen3-32B OLD-6 macro experiments")
    ...
    p.add_argument(
        "--qwen-only",
        action="store_true",
        help="Only wait for Qwen3-32B (skip Kimi probe). Default waits for both.",
    )
