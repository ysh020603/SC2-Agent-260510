"""Run prompt-integrated v9 if v8 misses the MediumHard target."""
...
PREVIOUS_BATCH = "human_skill_deepseek_flash_nothinking_fullv8_60_mediumhard_20260811"
MEDIUM_BATCH = "human_skill_deepseek_flash_nothinking_fullv9_60_mediumhard_20260811"
HARD_BATCH = "human_skill_deepseek_flash_nothinking_fullv9_60_hard_20260811"
TARGET_WINS = 30
...
def analyze(batch: str, suffix: str) -> tuple[Path, dict]:
    output = ROOT / "game_records/_human_skill_ablation" / batch / f"analysis_full_v9_{suffix}_60.json"
    subprocess.run(
        [str(PYTHON), "tools/analyze_human_skill_ablation.py", "--batch-prefix", batch,
         "--methods", "full_v9", "--baseline", "full_v9", "--output", str(output)],
        cwd=ROOT, check=True,
    )
    report = json.loads(output.read_text(encoding="utf-8"))
    return output, report["aggregate"]["full_v9"]


def run_topology(batch: str, difficulty: str, offset: int) -> int:
    command = [
        str(PYTHON), "tools/run_human_skill_reference_topology.py",
        "--method", "full_v9", "--model", "DeepSeek-V4-flash",
...
def main() -> int:
    ...
    parser.add_argument("--v8-pid", type=int, required=True)
    args = parser.parse_args()
    status_path = ROOT / "game_records/_human_skill_ablation" / MEDIUM_BATCH / "iteration_status.json"
    state = {"state": "waiting_for_v8", "target_wins": TARGET_WINS, "v8_pid": args.v8_pid}
    atomic_json(status_path, state)
    while process_matches(args.v8_pid, "run_v8_mediumhard_then_hard.py"):
        time.sleep(30)

    previous = json.loads(
        (ROOT / "game_records/_human_skill_ablation" / PREVIOUS_BATCH / "iteration_status.json").read_text(
            encoding="utf-8"
        )
    )
    state["v8_state"] = previous.get("state")
    state["v8_mediumhard"] = previous.get("mediumhard", {})
    if previous.get("state") != "mediumhard_below_target_iteration_required":
        state["state"] = "not_started_v8_met_target_or_continued_to_hard"
        atomic_json(status_path, state)
        return 0
    ...
    if state["mediumhard"]["n"] != 60:
        state["state"] = "blocked_v9_mediumhard_not_60_clean"
        atomic_json(status_path, state)
        return 2
    ...
    state["state"] = "complete" if state["hard"]["n"] == 60 else "blocked_v9_hard_not_60_clean"
    atomic_json(status_path, state)
