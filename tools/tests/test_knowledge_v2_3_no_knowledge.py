...
from SC2_Agent.knowledge_v2_3 import (
    build_knowledge_decision_context as build_v23_context,
)
from SC2_Agent.knowledge_v2_3.main_agent import MainAgent as V23MainAgent
from SC2_Agent.knowledge_v2_3.planner import (
    stabilize_queue_constraints as v23_stabilize_queue,
)
from SC2_Agent.knowledge_v2_3_no_knowledge import (
    build_knowledge_decision_context as build_control_context,
)
from SC2_Agent.knowledge_v2_3_no_knowledge import agent as control_agent
from SC2_Agent.knowledge_v2_3_no_knowledge.sub_agent import DataSubAgent
...
def test_no_knowledge_mode_is_additive_and_preserves_v23():
    ...
    assert build_control_context is build_v23_context
    assert control_agent.MainAgent is V23MainAgent
    assert control_agent.stabilize_queue_constraints is v23_stabilize_queue
...
def test_no_knowledge_subagent_answers_once_without_tools():
    ...
    session = DataSubAgent(invoker, recorder).run(
        {
            ...
            "sub_question": "Which Zerg unit can attack Battlecruiser?",
        },
        2,
    )
    ...
    assert "no-retrieval control condition" in messages[0]["content"]
    ...
    assert "knowledge_packet" not in session["reply"]
    assert not any(
