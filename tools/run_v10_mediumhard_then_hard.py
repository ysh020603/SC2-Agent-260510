"""Run opening-champion v10 if v9 misses the MediumHard target."""
...
PREVIOUS_BATCH = "human_skill_deepseek_flash_nothinking_fullv9_60_mediumhard_20260811"
MEDIUM_BATCH = "human_skill_deepseek_flash_nothinking_fullv10_60_mediumhard_20260811"
HARD_BATCH = "human_skill_deepseek_flash_nothinking_fullv10_60_hard_20260811"
TARGET_WINS = 30
METHOD = "full_v10"
VERSION = "v10"
PREVIOUS_PROCESS_FRAGMENT = "run_v9_mediumhard_then_hard.py"
...
def summarize(aggregate: dict) -> dict:
    ...
    return {"n": n, "wins": wins, "ties": int(results.get("Tie", 0)),
...
def analyze(batch: str, suffix: str) -> tuple[Path, dict]:
    ...
    subprocess.run([str(PYTHON), "tools/analyze_human_skill_ablation.py", "--batch-prefix", batch,
...
def run_topology(batch: str, difficulty: str, offset: int) -> int:
    command = [str(PYTHON), "tools/run_human_skill_reference_topology.py", "--method", METHOD,
...
def main() -> int:
    ...
    state = {"state": "waiting_for_v9", "target_wins": TARGET_WINS, "v9_pid": args.v9_pid}
    atomic_json(status_path, state)
    while process_matches(args.v9_pid, PREVIOUS_PROCESS_FRAGMENT):
        time.sleep(30)
    previous = json.loads((ROOT / "game_records/_human_skill_ablation" / PREVIOUS_BATCH / "iteration_status.json").read_text(encoding="utf-8"))
    state.update({"v9_state": previous.get("state"), "v9_mediumhard": previous.get("mediumhard", {})})
    if previous.get("state") != "mediumhard_below_target_iteration_required":
        state["state"] = "not_started_v9_met_target_or_continued_to_hard"
        atomic_json(status_path, state)
        return 0
    ...
    if state["mediumhard"]["n"] != 60:
        state["state"] = "blocked_v10_mediumhard_not_60_clean"
        atomic_json(status_path, state)
        return 2
    ...
    state["state"] = "complete" if state["hard"]["n"] == 60 else "blocked_v10_hard_not_60_clean"
    atomic_json(status_path, state)
