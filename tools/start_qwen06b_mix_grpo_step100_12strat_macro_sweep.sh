#!/usr/bin/env bash
# 12 Terran strategies vs 3 races (macro) @ KairosJunctionLE
# All modules: Qwen3-0.6b-sc2-mix-grpo-step100 (thinking / content_think_tags)
# Experiment matrix mirrors kimi_all (kimi_nothink_12strat_macro_r5).
#
# Usage:
#   bash tools/start_qwen06b_mix_grpo_step100_12strat_macro_sweep.sh
# Resume: CONCURRENCY=5 SKIP_COMPLETED=1 bash tools/start_qwen06b_mix_grpo_step100_12strat_macro_sweep.sh
...
MODEL_KEY="${MODEL_KEY:-Qwen3-0.6b-sc2-mix-grpo-step100_think}"
NAMING_MODEL="${NAMING_MODEL:-${MODEL_KEY}}"
ORDERING_MODEL="${ORDERING_MODEL:-${MODEL_KEY}}"
EXECUTOR_MODEL="${EXECUTOR_MODEL:-${MODEL_KEY}}"

SESSION="${TMUX_SESSION:-qwen06b_mix_grpo_step100_12strat_macro_sweep}"
BATCH="${BATCH_NAME:-qwen06b_mix_grpo_step100_12strat_macro_r5}"
TOTAL_JOBS=540
...
echo "=================================================="
echo " Started : mix-grpo-step100 0.6b (all modules, thinking)"
echo " Tmux    : $SESSION"
