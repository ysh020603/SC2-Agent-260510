"""Run 60 clean MediumHard games for V15, then 60 Hard games after the 30-win gate."""
...
MEDIUM_BATCH = "human_skill_deepseek_flash_nothinking_fullv15_60_mediumhard_20260812"
HARD_BATCH = "human_skill_deepseek_flash_nothinking_fullv15_60_hard_20260812"
METHOD = "full_v15"
VERSION = "v15"
TARGET_WINS = 30
...
def main() -> int:
    ...
    state = {
        "state": "starting_v15_mediumhard", "target_wins": TARGET_WINS,
        "design": "knowledge-constrained executable graph with generic producer, renewable-resource, prerequisite, and target-domain repair",
    }
    atomic_json(status_path, state)
    ...
    if state["mediumhard"]["n"] != 60:
        state["state"] = "blocked_v15_mediumhard_not_60_clean"
        atomic_json(status_path, state)
        return 2
    ...
    state["state"] = "complete" if state["hard"]["n"] == 60 else "blocked_v15_hard_not_60_clean"
    atomic_json(status_path, state)
