#!/usr/bin/env bash
# Qwen35-27b_think x 6 strategies vs Terran Macro @ KairosJunctionLE
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
cd "$ROOT"
SESSION="${TMUX_SESSION:-qwen35_think_terran_macro_sweep}"
BATCH_NAME="${BATCH_NAME:-qwen35_think_terran_macro_v6}"
MODEL="${MODEL:-Qwen35-27b_think}"
...
CONCURRENCY=$(printf '%q' "${CONCURRENCY:-2}") \
...
echo " Models       : ${MODEL} (naming/ordering/executor, thinking)"
echo " Concurrency  : ${CONCURRENCY:-2} | Jobs: 90"
