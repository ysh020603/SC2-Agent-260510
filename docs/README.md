...
| [system-architecture.md](system-architecture.md) | 理解当前 LLM 宏观流水线、`three-stage` / `two-stage` 决策模式、按对手种族加载策略 md、DATA_TOOLS 映射、`ExecutionScheduler`、BO list 模式和对局记录格式。 |
...
| [test-run-workflow.md](test-run-workflow.md) | 查找测试命令、两阶段 Kimi non-thinking 冒烟、运行流程、实验复现线索和测试脚本归档规则，适合检查输出文件或复跑对局时使用。 |
