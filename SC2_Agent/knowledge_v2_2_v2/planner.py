...
def _build_task_decomposition(
    *,
    ...
    resource_overflow = minerals >= 1000 and (bank_rising or minerals >= 2000)
    layer_gap = bool(layer_profile.get("air_attack_gap") or layer_profile.get("ground_attack_gap"))

    spend_pressure_per_minute = mineral_income + max(0.0, minerals - 600.0) * 60.0 / max(30.0, horizon)
...
def build_planning_snapshot(
    ...
    *,
    ...
    target_mineral_commitment = (
        min(2000.0, max(600.0, minerals * 0.35))
        if minerals >= 1000 and (bank_rising or minerals >= 2000)
        else 0.0
    )
    combat = observation.get("combat") if isinstance(observation.get("combat"), dict) else {}
...
def stabilize_queue_constraints(
    ...
    *,
    ...
    if should_fill and target_minerals > 0 and fallback_candidates:
        for _ in range(20):
            audit = build_queue_audit(names, planning_snapshot, data_path=data_path)
            if _number((audit.get("planned_cost") or {}).get("minerals")) >= target_minerals:
                break
            ...
            if not added:
                break
            corrections.append({"type": "fill_executable_mobile_capacity", "name": added})
