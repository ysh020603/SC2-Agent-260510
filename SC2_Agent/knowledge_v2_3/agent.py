...
def run_decision(
    *,
    ...
    try:
        ...
        knowledge_preferences_by_layer: dict[str, list[str]] = {
            "air": [],
            "ground": [],
        }
        knowledge_corrections: list[dict[str, Any]] = []

        def capture_knowledge_preferences(session: dict[str, Any]) -> None:
            if str(session.get("query_type") or "") not in {
                "enemy_counter", "combat_capability"
            }:
                return
            packet = ((session.get("reply") or {}).get("knowledge_packet") or {})
            guidance = packet.get("decision_guidance") or {}
            if not guidance.get("actionable"):
                return
            preferred = list(guidance.get("preferred_mobile_now") or [])
            preferred.extend(guidance.get("preferred_mobile_this_horizon") or [])
            preferred.extend(guidance.get("emergency_static_only") or [])
            for name in preferred:
                value = str(name)
                if value and value not in knowledge_preferences:
                    knowledge_preferences.append(value)
            for layer_name, values in (
                guidance.get("preferred_mobile_by_layer") or {}
            ).items():
                layer_key = str(layer_name).lower()
                if layer_key not in knowledge_preferences_by_layer:
                    continue
                for name in values or []:
                    value = str(name)
                    if value and value not in knowledge_preferences_by_layer[layer_key]:
                        knowledge_preferences_by_layer[layer_key].append(value)
        ...
        def ask_with_cache(decision: dict[str, Any], main_round: int) -> dict[str, Any]:
            ...
            if cached is not None:
                ...
                capture_knowledge_preferences(session)
        ...
        def validate_final(decision: dict[str, Any]) -> dict[str, Any] | None:
            normalized_names, corrections = stabilize_queue_constraints(
                ...
                knowledge_preferences_by_layer=knowledge_preferences_by_layer,
            )
            for correction in corrections:
                if (
                    correction.get("type") == "fill_verified_knowledge_response"
                    and correction not in knowledge_corrections
                ):
                    knowledge_corrections.append(dict(correction))
            ...
            # Composition counters are a high-value refinement, not a hard
            # blocker.  V2.2's count>=4 rule forced ceremonial Zergling/Marine
            # verification and made an infrastructure error abort production.
            # V2.3 only blocks on an unresolved direct weapon-layer gap above.
            ...
            "sessions": [
                {
                    ...
                    "recommended_shortlist": (
                        ((session.get("reply") or {}).get("knowledge_packet") or {}).get("recommended_shortlist")
                        or []
                    ),
                }
        ...
        result = {
            "agent_version": "decision-data-v2.3",
            ...
            "reasoning_policy": "per_role_api_profile_portable_text_tools",
            ...
            "routing": {
                "strategy": "v2_3_portable_repository_evidence_with_match_cache",
                "tool_protocol": "portable_local_orchestrator",
            ...
            "knowledge_application": (
                list(decision.get("knowledge_application") or [])
