"""Shared naive-equivalent domain prompt for SunTzu structural baseline.

Copied from SC2_Agent/decision_agent.py and adapted to expose
build_decision_context() without importing the live naive module as the
...
def _build_system_prompt(
    *,
    ...
    return f"""[1. Agent Role And Responsibility Boundary]
...
Public reason must be a concise 1-3 sentence explanation, not chain-of-thought,
hidden reasoning, or step-by-step thought process. Use only exact canonical
...
def build_decision_context(
    ...
    **_extra: Any,
    ...
    """Build the frozen domain context shared by Planner and Executor."""
    system_prompt = _build_system_prompt(
        ...
        strategy_automation_context=strategy_automation_context,
        decision_interval_seconds=decision_interval_seconds,
    )
    decision_event = _build_decision_event(
    ...
    return {
        ...
        "metadata": {
            ...
            "unfinished_canonical_names": list(unfinished_canonical_names),
            "canonical_unit_names": list(canonical_unit_names),
            "canonical_upgrade_names": list(canonical_upgrade_names),
            "mode": "suntzu",
        },
    }
...
def parse_decision_response(text: str) -> Optional[MacroDecision]:
    """Parse and validate a public decision / executor fragment response."""
