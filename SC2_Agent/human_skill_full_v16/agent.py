...
class HumanSkillFullV16Agent(HumanSkillAgent):
    """Knowledge-overlay graph with capability-chain and throughput repair."""

    def __init__(self, **kwargs):
        skill = kwargs.get("skill")
        if skill is None:
            raise ValueError("full-v16 requires a loaded skill")
        super().__init__(
    ...
    def _variant_transform_ordered_names(self, *, race, obs_text, ordered_names, game_time_seconds):
        return saturate_combat_queue(
            ...
            repair_level=max(0, self._conversion_streak - 1),
        )
