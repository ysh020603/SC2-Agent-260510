...
- Distinguish completed entities from entities under construction or merely queued. Pending technology is evidence that a transition has started, not evidence that its dependent action is executable now.
- Size parallel production from live income, unit cost, and database training time. A queue that spends less per minute than current income is not a repaired conversion loop even when every listed action is individually legal.
- Put a missing capability chain immediately after a small executable defense batch. At the next observation verify that its first missing prerequisite actually completed before scheduling dependent actions.
