...
class LiveState:
    ...
    pending_entities: frozenset[str]
    ...
    def future_count(self, name: str) -> int:
        key = name.upper()
        return self.count(name) + dict(self.pending_counts).get(key, 0)
...
def parse_live_state(obs_text: str) -> LiveState | None:
    ...
    def section_counts(label: str) -> dict[str, int]:
        match = re.search(rf"{re.escape(label)}:\s*([^\n]+)", own_text, re.I)
        rows: dict[str, int] = {}
        if not match or match.group(1).strip().lower().startswith("none"):
            return rows
        for raw_count, raw_name in re.findall(r"\b([0-9]+)\s+([A-Z][A-Z0-9_]*)\b", match.group(1)):
            name = raw_name.upper()
            rows[name] = rows.get(name, 0) + int(raw_count)
        return rows

    counts = section_counts("Completed")
    pending: dict[str, int] = {}
    for label in ("Under Construction", "Workers En Route", "Active Queues"):
        for name, count in section_counts(label).items():
    ...
    return LiveState(
        ...
        pending_entities=frozenset(pending),
        pending_counts=tuple(sorted(pending.items())),
    )
...
def conversion_alert(obs_text: str, game_time_seconds: float) -> str:
    ...
    if game_time_seconds >= 600 and state.bank >= 1400 and state.army_supply < 45:
...
def _build_batch(state: LiveState, profiles: list[CombatProfile]) -> list[CombatProfile]:
    ...
    target_spend = min(4000, max(900, int(state.bank * 0.75)))
    batch: list[CombatProfile] = []
    spent_minerals = spent_gas = 0
    cursor = 0
    misses = 0
    while len(batch) < 24 and (len(batch) < 8 or spent_minerals + spent_gas < target_spend):
...
def _parallel_producer_repair(race: str, state: LiveState, profiles: list[CombatProfile], batch_size: int) -> list[str]:
    ...
    if not producer:
        return []
    existing = state.future_count(producer)
    producer_profiles = [
        profile
        for profile in profiles
    ...
    live_income = state.mineral_income + state.gas_income
    throughput_need = int(math.ceil(max(0.0, live_income * 0.9) / max(1.0, spend_per_producer)))
    desired = min(8, max(2, int(math.ceil(batch_size / 4)), throughput_need))
    missing = min(4, max(0, desired - existing))
...
def _renewable_resource_repair(race: str, state: LiveState, main_profiles: list[CombatProfile]) -> list[str]:
    ...
    if (
        state.bank >= 1000
        and state.future_count(townhall) < 4
...
def _choose_requirement(race: str, alternatives: list[str], state: LiveState, planned: set[str]) -> str:
    ...
    for raw_name in alternatives:
        ...
        if not name:
            continue
        if name.upper() in state.pending_entities:
            continue
...
def _plan_entity_chain(
    ...
    *,
    ...
    if not target or target.upper() in state.own_entities or target in planned:
        return planned
    if target.upper() in state.pending_entities:
        return planned
...
def saturate_combat_queue(
    *,
    ...
    immediate_count = min(2 if capability_plan else 4, len(fitted))
